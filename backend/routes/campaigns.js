...
function buildFlowStatus(entries = [], requiredSteps = [], reviewState = {}) {
  ...
  for (const entry of entries) {
    const stepId = normalizeStepId(entry?.step);
    if (stepId) stepSet.add(stepId);
    const ts = Number(entry?.createdAt || entry?.uploadedAt);
    if (Number.isFinite(ts)) {
      if (!lastUploadAt || ts > lastUploadAt) lastUploadAt = ts;
      if (requiredSteps.includes(stepId) && (!lastRequiredAt || ts > lastRequiredAt)) {
        lastRequiredAt = ts;
      }
    }
  }

  const completed = requiredSteps.every(stepId => stepSet.has(stepId));
  ...
  return {
    ...
    completed,
    completedAt: completed ? lastRequiredAt || lastUploadAt || null : null,
    pendingSteps: requiredSteps.filter(stepId => !stepSet.has(stepId)),
