...
import { DRIVER_FLOW, GRAPHIC_FLOW } from '../lib/flows.js';
...
router.post('/graphic', validateGraphicLogin, async (req, res) => {
  ...
  if (!campaign) {
    return res.status(404).json({ error: 'Campanha nao encontrada para o codigo informado.' });
  }

  const storedCode = String(campaign.campaignCode || '').toUpperCase();
  if (!storedCode) {
    return res.status(403).json({ error: 'Codigo de acesso ainda nao configurado para esta campanha.' });
  }
  if (storedCode !== normalizedCode) {
    return res.status(403).json({ error: 'Codigo da campanha invalido.' });
  }
  ...
  if (!graphics.length) {
    return res.status(403).json({ error: 'Nenhuma grafica cadastrada para esta campanha.' });
  }
  ...
  if (!match) {
    return res.status(403).json({ error: 'Nome do responsavel nao confere com a grafica cadastrada.' });
  }
  ...
  if (item.photoData) {
    try {
      ...
      const header = Array.isArray(campaign.sheetHeader) && campaign.sheetHeader.length
        ? campaign.sheetHeader
        : await readSheetHeader(campaign.sheetId, campaign.sheetName);
