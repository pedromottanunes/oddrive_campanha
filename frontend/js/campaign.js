...
function describeFlowState(flowStatus) {
  if (!flowStatus) return { className: 'is-pending', text: 'Sem envio' };
  if (flowStatus.verifiedAt) return { className: 'is-verified', text: 'Verificado' };
  if (flowStatus.completed) return { className: 'is-completed', text: 'Envio das imagens concluido' };
  if (flowStatus.hasUploads) return { className: 'is-progress', text: 'Envio em andamento' };
...
function buildStatusNote(flowStatus) {
  if (!flowStatus) return 'Nenhum envio registrado.';
  if (flowStatus.verifiedAt) {
    ...
    return when ? `Verificado em ${when} por ${reviewer}.` : `Verificado por ${reviewer}.`;
  }
  if (flowStatus.completed) {
    const when = flowStatus.completedAt ? formatDateTime(flowStatus.completedAt) : '';
    return when ? `Envio das imagens concluido em ${when}.` : 'Envio das imagens concluido.';
  }
  if (flowStatus.hasUploads) {
    const when = formatDateTime(flowStatus.lastUploadAt);
    const pending = formatPendingSteps(flowStatus.pendingSteps);
    return `${when ? `Ultimo envio em ${when}.` : 'Ultimo envio registrado.'}${pending ? ` Pendentes: ${pending}.` : ''}`;
  }
...
function setSelectedDriver(driver) {
  ...
  if (btnVerifyDriver) {
    ...
    btnVerifyDriver.disabled = !completed;
    btnVerifyDriver.textContent = verified ? 'Remover verificacao' : 'Marcar como verificado';
  }
  if (btnVerifyGraphic) {
    const completed = Boolean(graphicFlow?.completed);
    const verified = Boolean(graphicFlow?.verifiedAt);
    btnVerifyGraphic.disabled = !completed;
    btnVerifyGraphic.textContent = verified ? 'Remover verificacao' : 'Marcar como verificado';
  }
}

function updateDriverStatusChips(container, driver) {
  if (!container) return;
  const statuses = driver?.evidenceStatus || {};
...
function updateDriverListItemStatus(driver) {
  ...
  if (chips) updateDriverStatusChips(chips, driver);
}
...
async function confirmCampaignStatusChange(nextStatus) {
  ...
  if (typeof openAdminPrompt === 'function') {
    const result = await openAdminPrompt({
      ...
      description: `Deseja confirmar a mudança do status para "${prettyLabel}"?`,
  ...
  return window.confirm(`Deseja confirmar a mudança do status para "${prettyLabel}"?`);
}
...
function renderGalleryItems(items = []) {
  ...
  for (const it of list) {
    ...
    deleteBtn.addEventListener('click', async (e) => {
      e.stopPropagation();
      if (!confirm('Tem certeza que deseja deletar esta imagem? Esta ação não pode ser desfeita.')) return;
      try {
...
function renderGraphicStorageExplorer(
  ...
  const resolvedUploaderType = (uploaderType || tree.uploaderType || '')
    ...
    .trim()
    .toLowerCase();
  const type = resolvedUploaderType === 'graphic'
    ? 'graphic'
  ...
  folders.forEach(folder => {
    ...
    deleteBtn.addEventListener('click', async (e) => {
      ...
      if (!confirm(`Deletar toda a pasta "${formatStorageDateFolder(folder.name)}" com ${fileCount} arquivo(s)? Esta ação não pode ser desfeita.`)) return;
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (uploaderType === 'graphic') {
            const updatedTree = await fetchGraphicStorageTree(campaignId, driver.id);
            renderGraphicStorageExplorer(updatedTree, { driver, headingText, uploaderType });
          } else {
            const updatedTree = await fetchDriverStorageTree(campaignId, driver.id);
            renderGraphicStorageExplorer(updatedTree, { driver, headingText, uploaderType });
          }
        } else {
  ...
  if (!files.length) {
    ...
    files.forEach(file => {
      ...
      deleteBtn.onclick = async (e) => {
        ...
        if (!confirm(`Deletar o arquivo "${file.name}"?`)) return;
...
function renderDriverList(drivers = [], options = {}) {
  ...
  for (const d of list) {
    ...
    updateDriverStatusChips(chips, d);
      ...
      renderDriverList(drivers, {
        ...
        onDriverClick: async (driver) => {
          ...
          if (storageTree && Array.isArray(storageTree.folders) && storageTree.folders.length) {
            ...
            renderGalleryItems(itemsForDriver);
          } else {
            acompanheGalleryGrid.classList.add('is-explorer');
...
async function renderAcompanhe(data) {
  ...
  const mode = acompanheMode ? acompanheMode.value : 'driver';
  if (mode === 'graphic') {
...
function setupAcompanheUI() {
  ...
  if (btnCleanup) {
    btnCleanup.addEventListener('click', async () => {
      ...
      if (!confirm('Limpar evidências órfãs (referências a arquivos deletados)? Esta ação removerá registros que apontam para imagens que não existem mais.')) return;
...
function renderDriverDetails(driver) {
  ...
  const HIDE_KEYS = (() => {
    const base = [
      ...
      'COMENTARIOS','OBSERVACOES','PASTA DRIVE','_ATUALIZADO EM','_ORIGEM',
  ...
  if (tblGraphics) {
    tblGraphics.addEventListener('click', async event => {
      ...
      if (deleteButton) {
        ...
        if (!graphicId) return;
        if (!confirm('Deseja remover esta grafica?')) return;
