...
function showSuccess(message='Concluído com sucesso! Obrigado.') {
  ...
  overlayEl.classList.add('show');
  // show overlay briefly and then show a persistent success panel in the app
  setTimeout(() => {
    ...
    try {
      ...
      const btnRef = document.getElementById('btnRefazer');
      if (btnRef) btnRef.addEventListener('click', () => {
        try { stepData.clear(); } catch (e) {}
        isRefazer = true;
        currentStepIndex = 0;
        if (currentFlow) renderFlow(currentFlow);
        const p = document.getElementById('successPanel'); if (p) p.remove();
      });
        // after showing success, clear the refazer flag so subsequent cycles are normal
        isRefazer = false;
